...
from ray.data._internal.logical.operators import Aggregate, MapGroupPartitions
...
class MapGroupPartitionsContext:
    """Information about the groups contained in one input partition."""
    ...
    def num_groups(self) -> int:
        """Number of complete groups in this input partition."""
...
class GroupedData:
    ...
    @PublicAPI(api_group=FA_API_GROUP, stability="alpha")
    def map_group_partitions(
        self,
        fn: Callable[..., Union[DataBatch, Iterator[DataBatch]]],
    ) -> "Dataset":
        """Apply an optimized function to partitions containing complete groups.

        This method is an explicit fast path for a Parquet read followed by a cuDF
        ``map_batches`` operation. Ray range-partitions the input by the leading
        group key, sorts each partition by all group keys, and calls ``fn`` once per
        nonempty partition. Ray passes ``fn`` the cuDF partition and a
        :class:`MapGroupPartitionsContext` describing the complete groups in it.

        Unlike :meth:`map_groups`, this method doesn't provide a general-purpose
        fallback. Ray raises an error if the upstream pipeline or data doesn't
        satisfy the fast-path contract.

        The upstream tokenizer must be synchronous, deterministic and retry-safe,
        invariant to batch boundaries and composition, and return cuDF DataFrames.
        It may change row count, but each output row must retain every group key and
        keep its leading key in the actor's assigned range.

        Args:
            fn: A function that accepts a cuDF batch and a
                :class:`MapGroupPartitionsContext`, and returns one output batch or
                an iterator of output batches.

        Returns:
            A dataset containing the combined outputs from all group partitions.
        """
        if not callable(fn):
            raise TypeError(f"fn must be callable, got {type(fn)!r}")
        if not isinstance(self._key, (str, list)) or not self._key:
            raise ValueError(
                "map_group_partitions requires one or more named group-by columns"
            )
        if isinstance(self._key, list) and any(
            not isinstance(key, str) or not key for key in self._key
        ):
            raise ValueError(
                "map_group_partitions requires non-empty string group-by columns"
            )
