...
class TestGPURankPool:
    ...
    def test_activate_uses_exact_bundle_indexes_and_poll_ready_waits_for_setup(self):
        ...
        actor_handles[0].setup_root.remote.return_value = root_ref
        ...
        with (
            ...
            mock_ray_wait.side_effect = [([pg_ready_ref], []), (worker_refs, [])]
    ...
    def test_setup_failure_cleans_up_every_rank_and_placement_group(self):
        ...
        pool._actors = actors
        pool._worker_setup_refs = worker_refs
    ...
    def test_shutdown_retains_failed_handles_for_retry(self):
        ...
        with (
            ...
            assert pool._placement_group is placement_group
            assert pool.state is not GPURankPoolState.CLOSED
            pool.shutdown()
...
class TestGPUShuffleOperatorConstructor:
    ...
    def test_frontier_grant_acquires_gang_without_accepting_input(self):
        ...
        with (
            ...
            patch.object(op._rank_pool, "poll_ready") as mock_poll_ready,
        ):
            op.apply_resource_admission_grant(
            ...
            mock_poll_ready.assert_not_called()
...
class TestGPUShuffleOperatorInputRouting:
    ...
    def test_idle_gang_can_be_revoked_before_first_input(self):
        op, _ = self._make_op_with_mock_pool(nranks=2)
        op._admission_grant = ResourceAdmissionGrant(
    ...
    def test_gang_is_sticky_after_first_input(self):
        op, _ = self._make_op_with_mock_pool(nranks=2)
        op._admission_grant = ResourceAdmissionGrant(
            max_units=1, may_submit=True
        )
        ...
        assert op._admission_grant.max_units == 1
...
class TestGPUShuffleOperatorFinalization:
    ...
    def test_empty_input_finalization_cannot_be_revoked(self):
        op, _ = self._make_op(nranks=2)
        op._admission_grant = ResourceAdmissionGrant(
            max_units=1, may_submit=True
        )
    ...
    def test_pending_rank_setup_refs_are_active_tasks(self):
        ...
        op._rank_pool._worker_setup_refs = setup_refs
...
class TestGPUShuffleActorReal:
    ...
    def test_setup_root_returns_address(self, ray_with_gpu):
        """setup_root() returns the address bytes needed for UCXX setup."""
        actor = GPUShuffleActor.options(num_gpus=1).remote(
            nranks=1, total_nparts=1, key_columns=["k"]
        )
        addr = ray.get(actor.setup_root.remote())
