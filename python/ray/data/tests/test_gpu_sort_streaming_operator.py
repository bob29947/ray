...
class _Bundle:
    def __init__(self, block_ref, *, size_bytes: int, num_rows: int):
        ...
        self.blocks = ((block_ref, metadata),)
        self.slices = (None,)
        self.schema = None
...
def test_start_starts_gpu_ranks_before_accepting_input():
    ...
    operator.start(ExecutionOptions(preserve_order=True))
...
def test_start_requires_preserved_input_order():
    ...
    with pytest.raises(RuntimeError, match="preserve_order=True"):
        operator.start(ExecutionOptions(preserve_order=False))
