...
class TokenizerPipeline:
    ...
    def fit(self, df: cudf.DataFrame) -> "TokenizerPipeline":
        ...
        self._assign_step_offsets()
        self.is_fitted = True
        return self

    def _assign_step_offsets(self) -> None:
        """Push each step's global vocab offset onto the step so the on-device
        ``tokenize_ids`` path can emit final global IDs (``offset + local_key``).
        Steps in this pipeline use flat (non-sub-dict) vocabs, so the offset is
        simply ``vocab_offset[tok_id]``.
        """
        for tok_id in self.tokenizer_order:
            self.steps[tok_id].global_offset = int(self.vocab_offset.get(tok_id, 0))
    ...
    # ------------------------------------------------------------------
    # Transform  ->  on-device int32 token-ID matrix (GPU fast path)
    # ------------------------------------------------------------------
    def transform_ids(self, df: cudf.DataFrame) -> "cp.ndarray":
        """Tokenize directly to a ``(n_rows, n_fields)`` int32 **cupy** matrix of
        global token IDs, entirely on the GPU.

        This is the fast path used by the fused GPU read+tokenize operator: every
        step emits integer IDs via ``tokenize_ids`` (no token strings, no
        ``.to_pandas().map(vocab)`` host round-trips), and the columns are stacked on
        the device. The result stays in VRAM; the caller decides if/when to copy to
        host. Column order matches ``tokenizer_order`` (and the string ``transform``).
        """
        if not self.is_fitted:
            raise ValueError("Must call fit() before transform_ids()")
        self._assign_step_offsets()
        cols = []
        for tok_id in self.tokenizer_order:
            columns = self.column_specs[tok_id]
            col_data = df[columns] if len(columns) > 1 else df[columns[0]]
            ids = self.steps[tok_id].tokenize_ids(col_data)  # int32 cudf.Series
            cols.append(ids.values)  # cupy int32 array (zero-copy view of the Series)
        return cp.stack(cols, axis=1).astype(cp.int32)  # (n_rows, n_fields) on device
