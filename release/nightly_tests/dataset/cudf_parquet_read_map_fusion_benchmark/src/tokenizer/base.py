...
class BaseTokenizer(ABC):
    ...
    def __init__(self):
        ...
        # Global vocab offset for the on-device integer-ID path. The pipeline sets
        # this after fit() so that ``tokenize_ids`` can emit final global token IDs
        # (``global_offset + local_key``) directly on the GPU, matching exactly the
        # string path's ``vocab[token_str]`` (see TokenizerPipeline._build_global_vocab,
        # where ``gid = local_key + offset``). 0 until the pipeline assigns it.
    ...
    def tokenize_ids(self, column_data) -> "cudf.Series":
        """Map column values directly to **global** integer token IDs on the GPU.

        Returns an ``int32`` cudf.Series of ``global_offset + local_key`` with no
        intermediate token strings and no host round-trips. Subclasses on the hot
        path override this; the default mirrors ``tokenize`` only as a (slow)
        fallback via the host vocab and is not used by the GPU fast path.
        """
        raise NotImplementedError(
            f"{type(self).__name__} does not implement the on-device tokenize_ids path"
        )
