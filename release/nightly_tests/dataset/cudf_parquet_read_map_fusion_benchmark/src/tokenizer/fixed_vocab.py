...
class FixedVocabTokenizer(BaseTokenizer):
    ...
    def tokenize_ids(self, column_data) -> cudf.Series:
        """On-device global IDs: ``global_offset + clip(value, min, max)``.

        The string path's ``_idx_to_token`` is keyed by the raw value ``i`` in
        ``[min_val, max_val]`` and the global vocab uses ``gid = i + offset``, so the
        clipped integer value *is* the local key. Pure cuDF arithmetic, no strings.
        """
        int_vals = column_data.astype("int32").clip(self.min_val, self.max_val)
        return (int_vals + self.global_offset).astype("int32")
