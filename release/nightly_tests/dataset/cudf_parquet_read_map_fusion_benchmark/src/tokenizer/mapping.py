...
class MappingTokenizer(BaseTokenizer):
    ...
    # ------------------------------------------------------------------
    # On-device integer-ID path (GPU fast path)
    # ------------------------------------------------------------------
    @staticmethod
    ...
    def _build_id_lookup(self) -> None:
        """Derive value -> local-index maps from ``_idx_to_token`` (built by
        ``build_vocab``) so ``tokenize_ids`` can emit global IDs on the GPU with no
        token strings. The local index is exactly the ``_idx_to_token`` key, so
        ``gid = global_offset + local_index`` matches the string vocab byte-for-byte.

        Three order-preserving device fast paths are precomputed:
          * range mode  -> sorted upper bounds + per-range codes for ``searchsorted``.
          * integer-keyed direct/passthrough (e.g. MCC = ``str(int)``) -> a dense
            ``value -> code`` lookup table for a single gather.
          * genuine string keys (CHIP, STATE) -> a cuDF lookup Series for ``.map``.
        """
        if getattr(self, "_id_lookup_built", False):
            return
        plen = len(self.prefix) + 1  # strip "PREFIX_" (label itself may contain "_")
        idx_to_label = {i: tok[plen:] for i, tok in self._idx_to_token.items()}
        label_to_idx = {lbl: i for i, lbl in idx_to_label.items()}
        self._default_idx = int(label_to_idx[self.default])

        if self._mode == "range":
            ranges = sorted(self._range_lookup, key=lambda r: int(r[0]))
            self._rng_min = int(ranges[0][0])
            self._rng_max = int(ranges[-1][1])
            self._rng_bounds = cp.asarray(
                [int(hi) for _, hi, _ in ranges], dtype=cp.int64
            )
            self._rng_codes = cp.asarray(
                [int(label_to_idx[lbl]) for _, _, lbl in ranges], dtype=cp.int32
            )
            self._int_mode = None
        else:
            keys = [str(k) for k in self._direct_map.keys()]
            if keys and all(self._is_intlike(k) for k in keys):
                # Dense integer lookup table: idx = value - min  ->  local code.
                self._int_mode = True
                ivals = [int(k) for k in keys]
                self._int_min = min(ivals)
                self._int_max = max(ivals)
                size = self._int_max - self._int_min + 1
                lut = np.full(size, self._default_idx, dtype=np.int32)
                for raw, lbl in self._direct_map.items():
                    lut[int(str(raw)) - self._int_min] = int(label_to_idx[str(lbl)])
                self._int_lut = cp.asarray(lut)
            else:
                # String keys: cuDF lookup Series (value -> code) for order-preserving map.
                self._int_mode = False
                self._map_lut = cudf.Series(
                    cp.asarray(
                        [int(label_to_idx[str(v)]) for v in self._direct_map.values()],
                        dtype=cp.int32,
                    ),
                    index=[str(k) for k in self._direct_map.keys()],
                )
        self._id_lookup_built = True

    def tokenize_ids(self, column_data) -> cudf.Series:
        """On-device global integer IDs (``int32``), no token strings, no host hop."""
        self._build_id_lookup()
        off = self.global_offset
        idx = column_data.index

        if self._mode == "range":
            v = column_data.astype("int64").values  # cupy int64
            pos = cp.clip(
                cp.searchsorted(self._rng_bounds, v, side="left"),
                0,
                self._rng_bounds.shape[0] - 1,
            )
            code = self._rng_codes[pos]
            valid = (v >= self._rng_min) & (v <= self._rng_max)
            out = cp.where(valid, code, self._default_idx).astype(cp.int32)
            return cudf.Series(out + off, index=idx).astype("int32")

        if self._int_mode:
            v = column_data.astype("int64").values
            gi = v - self._int_min
            valid = (gi >= 0) & (gi < self._int_lut.shape[0])
            code = self._int_lut[cp.clip(gi, 0, self._int_lut.shape[0] - 1)]
            out = cp.where(valid, code, self._default_idx).astype(cp.int32)
            return cudf.Series(out + off, index=idx).astype("int32")

        # genuine string keys: order-preserving map via a cuDF lookup Series.
        mapped = column_data.astype("str").map(self._map_lut)
        out = mapped.fillna(self._default_idx).astype("int32") + off
    ...
    def _tokenize_direct(self, column_data) -> cudf.Series:
        ...
        s = (
            ...
            if not hasattr(column_data, "str") or column_data.dtype != "object"
            else column_data
        )
        host = s.to_pandas()
        if self._direct_map:
    ...
    def _tokenize_range(self, column_data) -> cudf.Series:
        ...
        vals = (
            column_data.values.get()
            if hasattr(column_data.values, "get")
            else column_data.values
        )
    ...
    def __repr__(self) -> str:
        ...
        return f"MappingTokenizer(prefix={self.prefix}, mode={self._mode}, {status})"
