...
class FinancialTokenizerPipeline(TokenizerPipeline):
    ...
    def preprocess(
        ...
        ----------
        sort : bool
            Sort by (user, card, time_full). Needed for sequence assembly
            (NB02), but the read+tokenize fast path skips it (downstream
            ``build_sequences`` re-sorts by ts), avoiding a full-batch GPU sort.
        compute_time_delta : bool
            Compute the per-(user, card) ``time_delta_s`` via a groupby-diff.
            Off in the fast path: it is unused by the default 12-field vocab and
            a groupby-diff over the whole batch is pure overhead there.
        """
        ...
        if fast_dates:
            # Integer-only date math (no string build, no to_datetime parse): hour/
            # minute by slicing the zero-padded "HH:MM" time, month from the input
            # column, day-of-week via Zeller's congruence, and epoch seconds via the
            # civil-days algorithm. All validated bit-exact against cudf.to_datetime.
            t = df["time"].fillna("00:00").astype(str)
            hh = t.str.slice(0, 2).astype("int64").values
            mm = t.str.slice(3, 5).astype("int64").values
            y = df["year"].astype("int64").values
            mo = df["month"].astype("int64").values
            d = df["day"].astype("int64").values
            ms = cp.where(mo < 3, mo + 12, mo)
            ys = cp.where(mo < 3, y - 1, y)
            K = ys % 100
            J = ys // 100
            h = (d + (13 * (ms + 1)) // 5 + K + K // 4 + J // 4 + 5 * J) % 7  # 0=Sat
            df["hour"] = cudf.Series(hh, index=df.index).astype("int32")
            df["dow"] = cudf.Series(((h + 5) % 7), index=df.index).astype(
                "int32"
            )  # Mon=0
            df["month"] = df["month"].astype("int32")
            y2 = y - (mo <= 2).astype("int64")
            era = cp.where(y2 >= 0, y2, y2 - 399) // 400
            yoe = y2 - era * 400
            mp = cp.where(mo > 2, mo - 3, mo + 9)
            doy = (153 * mp + 2) // 5 + d - 1
            doe = yoe * 365 + yoe // 4 - yoe // 100 + doy
            days = era * 146097 + doe - 719468
            ts = days * 86400 + hh * 3600 + mm * 60  # epoch seconds (UTC)
            df["ts_epoch"] = cudf.Series(ts, index=df.index).astype("int64")
            dt = None
        else:
            date_str = (
                df["year"].astype(str)
        ...
        zip_col = (
            df["zip"].fillna("00000").astype(str).str.replace(".0", "", regex=False)
        )
        ...
        if dt is not None and "time_full" not in df.columns:
            df["time_full"] = dt
