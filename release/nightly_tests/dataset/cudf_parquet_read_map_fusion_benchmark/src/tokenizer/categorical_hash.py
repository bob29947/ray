...
class CategoricalHashTokenizer(BaseTokenizer):
    ...
    def build_vocab(self, column_data=None) -> None:
        ...
        if self.stream:
            with self.stream:
                ...
                self._idx_to_token = dict(zip(idx.to_pandas(), tokens.to_pandas()))
        else:
            self._idx_to_token = {
    ...
    def tokenize_ids(self, column_data) -> cudf.Series:
        """On-device global IDs: ``global_offset + (hash_value % vocab_limit)``.

        ``_idx_to_token`` is keyed ``0..vocab_limit-1`` with ``gid = key + offset``,
        so the hash bucket is the local key. *column_data* must already be integer
        hash values (cuDF ``hash_values()``); pure modulo + add on the GPU.
        """
        token_bucket = (column_data % self.vocab_limit).astype("int32")
        return (token_bucket + self.global_offset).astype("int32")
