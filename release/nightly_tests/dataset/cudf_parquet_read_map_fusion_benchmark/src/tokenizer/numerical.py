...
# NOTE (Ray edition): cuML is imported lazily inside __init__ so that importing
# the tokenizer package does NOT require a cuML install. The default financial
# pipeline uses fixed-bin amounts and never instantiates this class, keeping the
# GPU tokenization runtime_env lean (cuDF + CuPy only).
...
class NumericalTokenizerOptBin(BaseTokenizer):
    ...
    def __init__(
        ...
        from cuml.preprocessing import KBinsDiscretizer  # lazy: only when used
